...
private:
	...
	static ContaminationDetectionResult bootstrapTask(const SequenceVectorizationResult & svr, const std::vector<unsigned> indices);
...
public:
	...
	static ContaminationDetectionResult analyze(const Eigen::MatrixXd & data, const std::vector<std::string> & contigs, const std::map<std::string, unsigned> & contigSizes);
